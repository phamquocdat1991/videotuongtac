...
export default function App() {
  ...
  return (
    <div className="app-shell flex min-h-screen flex-col font-sans text-slate-100 antialiased selection:bg-cyan-400/30">
      ...
      {analysisError && <div className="border-b border-rose-500/30 bg-rose-950/80 px-4 py-3 text-rose-100"><div className="mx-auto flex max-w-[1480px] items-start justify-between gap-3 text-xs"><div className="flex items-start gap-2"><XCircle className="mt-0.5 h-4 w-4 flex-none text-rose-400" /><span><strong className="mr-2 text-rose-300">Phân tích đã dừng.</strong>{analysisError}</span></div><div className="flex items-center gap-2"><button type="button" onClick={() => setIsApiSettingsOpen(true)} className="flex items-center gap-1 rounded-lg bg-rose-800 px-2.5 py-1 font-bold"><Key className="h-3 w-3" />Cài đặt</button><button type="button" onClick={() => setAnalysisError(null)} aria-label="Đóng cảnh báo" className="rounded p-1 hover:bg-white/10">×</button></div></div></div>}
      ...
      <main className="mx-auto flex w-full max-w-[1480px] flex-1 flex-col gap-5 px-4 py-5 sm:px-6 lg:px-8 lg:py-7">
        <section className="flex flex-col gap-3 rounded-3xl border border-white/[0.07] bg-gradient-to-r from-white/[0.055] to-cyan-400/[0.025] px-5 py-5 shadow-[0_30px_80px_rgba(1,6,18,0.24)] sm:flex-row sm:items-end sm:justify-between">
          <div><div className="mb-2 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.2em] text-cyan-300"><Sparkles className="h-3.5 w-3.5" />Không gian thiết kế bài học</div><h2 className="max-w-3xl text-xl font-extrabold tracking-tight text-white sm:text-2xl">Biến video thành trải nghiệm học có chủ đích</h2><p className="mt-1 max-w-2xl text-xs leading-relaxed text-slate-400">Thêm mốc dừng, kiểm tra chất lượng và xuất bài giảng dùng ngay trên web hoặc LMS.</p></div>
          <div className="flex flex-wrap items-center gap-2"><span className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-black/10 px-3 py-1.5 text-[11px] text-slate-400"><Cloud className="h-3.5 w-3.5 text-emerald-300" />{saveLabel}</span><span className="rounded-full border border-white/10 bg-black/10 px-3 py-1.5 text-[11px] text-slate-300">{subject} · {grade}</span></div>
        </section>
        ...
        <div className="grid grid-cols-1 gap-5 xl:grid-cols-12 xl:items-start">
          <div className="flex flex-col gap-5 xl:col-span-5 xl:sticky xl:top-[92px]"><UploadSection apiKey={activeApiKey} provider={provider} selectedModel={selectedModel} onOpenApiSettings={() => setIsApiSettingsOpen(true)} videoFileName={videoFileName} videoUrl={videoUrl} videoDuration={videoDuration} onVideoSelected={handleVideoSelected} lessonMaterial={lessonMaterial} onLessonMaterialChange={setLessonMaterial} onLessonFileChange={setLessonFile} lessonText={lessonText} onLessonTextChange={setLessonText} subject={subject} onSubjectChange={setSubject} grade={grade} onGradeChange={setGrade} isAnalyzing={isAnalyzing} onAnalyze={handleAIAnalyze} /></div>
          <div className="flex min-w-0 flex-col gap-5 xl:col-span-7"><InteractivePlayerPreview videoUrl={videoUrl} videoFileName={videoFileName} interactions={interactions} seekTimestampTarget={seekTimestampTarget} onDurationDetected={setVideoDuration} />
      ...
      <footer className="border-t border-white/[0.06] px-6 py-5 text-[11px] text-slate-500"><div className="mx-auto flex max-w-[1480px] flex-col gap-1 sm:flex-row sm:items-center sm:justify-between"><span>Interactive Video Studio · Edu Edition v2.8</span><span>Thiết kế bởi <strong className="text-slate-300">PHẠM QUỐC ĐẠT</strong> · Zalo 0705350000</span></div></footer>
